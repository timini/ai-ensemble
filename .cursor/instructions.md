...
## 4. Playwright E2E Policy: No Network Stubbing/Mocking

- Do not stub or mock network in Playwright tests. This includes intercepting `fetch`, stubbing SSE streams, short-circuiting tRPC calls, or modifying responses in test code.
- Playwright E2E must use real/sandbox API keys provided via `.env.playwright` and exercise the product through its UI.
- If E2E flakiness is caused by elements intercepting clicks or timing, fix the underlying UI (z-index/focus/ARIA/selector) instead of adding brittle waits or mocks.
